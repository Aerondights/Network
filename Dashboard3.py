...
class VLANDashboard:
    ...
    def calculer_metriques_globales(self):
        ...
        col_used = next((col for col in self.df_vlan.columns if 'Used Size' in col), None)
        col_free = next((col for col in self.df_vlan.columns if 'free size' in col.lower()), None)
        col_percent = next((col for col in self.df_vlan.columns if 'Used percent' in col), None)
