...
class PSSITAuthenticator:
    def __init__(self, username, password):
        """
        Args:
            username: Nom d'utilisateur (format: user@corporate ou juste user)
    ...
    def get_saml_token(self):
        ...
        try:
            ...
            # Si on n'a pas encore l'URL Kerberos, c'est qu'elle est dans location
            if resp.status_code in [301, 302, 303, 307, 308]:
                krb_url = resp.headers.get('Location')
            else:
                krb_url = location
            
            # Si l'URL est relative, la rendre absolue
            if not krb_url.startswith('http'):
                krb_url = f"https://ca-collaboration.com{krb_url}"
            
            print(f"  URL Kerberos: {krb_url}")
            
            # Construire l'auth NTLM (avec format username@domain)
...
def get_saml_token_simple(username, password):
    """
    Version simplifiée pour le flux PSSIT spécifique
    """
    session = requests.Session()
    
    # Auth NTLM (la popup navigateur est remplacée par HttpNtlmAuth)
    auth = HttpNtlmAuth(username, password)
    
    # 1. Accéder à PSSIT qui redirige vers l'IdP avec auth Kerberos
    resp = session.get(
        'https://pssit.saas.cagip',
        auth=auth,
